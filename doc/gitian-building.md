...
*Setup instructions for a gitian build of Etern using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Etern
...
Clone the git repositories for etern and gitian and then checkout the etern version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/etern-crypto/etern.git
cd etern
...
Follow the instructions in [doc/release-process.md](release-process.md) in the etern repository
...
To build Etern (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the etern repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/etern/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/etern-crypto/etern
...
For example:
```bash
URL=https://github.com/crowning-/etern.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit etern=${COMMIT} --url etern=${URL} ../etern/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit etern=${COMMIT} --url etern=${URL} ../etern/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit etern=${COMMIT} --url etern=${URL} ../etern/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/etern-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/etern-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/etern-build.assert
```
...
[etern/gitian.sigs](https://github.com/etern-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
