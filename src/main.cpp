...
#if defined(NDEBUG)
#error "Etern cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ueten) are considered zero fee (for relaying and mining)
...
const string strMessageMagic = "Etern Signed Message:\n";
...
bool ContextualCheckCoinSpend(const CoinSpend& spend, CBlockIndex* pindex, const uint256& txid)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
            return error("%s : zEten with serial %s is already in the block %d\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zEten spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckCoinSpend(spend, chainActive.Tip(), txid))
                    return state.Invalid(error("%s: zETEN spend in tx %s failed to pass context checks", __func__, txid.GetHex()));
            }
        } else {
            LOCK(pool.cs);
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 500 * COIN;
    } else if (nHeight > 200 && nHeight <= 40000) {
        nSubsidy = 18 * COIN;
    } else if (nHeight > 40001 && nHeight <= 65000) {
        nSubsidy = 9 * COIN;
    } else if (nHeight > 65001 && nHeight <= 95000) {
        nSubsidy = 6 * COIN;
    } else if (nHeight > 95001 && nHeight <= 125000) {
        nSubsidy = 4 * COIN;
    } else if (nHeight > 125001 && nHeight <= 350000) {
        nSubsidy = 2 * COIN;
    } else if (nHeight > 350001 && nHeight <= 1480001) {
        nSubsidy = 0.80 * COIN;
    } else {
        nSubsidy = 0.80 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Etern
...
{
    RenameThread("etern-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZETENMinted()
{
...
void RecalculateZETENSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zETEN supply
...
bool RecalculateETENSupply(int nHeightStart)
{
...
bool UpdateZETENSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (Params().Zerocoin_Block_RecalculateAccumulators() >= 0 && pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateETENSupply(Params().Zerocoin_StartHeight());
    }

    //Track zETEN money supply in the block index
    if (!UpdateZETENSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zETEN supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zEtenSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert ETEN to zETEN
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zEten spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zEten serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zEten mints and %d zEten spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Etern: We use certain sporks during IBD, so check to see if they are
