...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::ETEN);
    ...
    if (!settings.contains("nAnonymizeEternAmount"))
        settings.setValue("nAnonymizeEternAmount", 1000);

    nAnonymizeEternAmount = settings.value("nAnonymizeEternAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeEternAmount"))
        SoftSetArg("-anonymizeeternamount", settings.value("nAnonymizeEternAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in etern.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeEternAmount:
            return QVariant(nAnonymizeEternAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeEternAmount:
            nAnonymizeEternAmount = value.toInt();
            settings.setValue("nAnonymizeEternAmount", nAnonymizeEternAmount);
            emit anonymizeEternAmountChanged(nAnonymizeEternAmount);
            break;
        case CoinControlFeatures:
