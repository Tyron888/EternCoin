...
#include "ui_zetencontroldialog.h"
...
std::list<std::string> ZEtenControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZEtenControlDialog::listMints;

ZEtenControlDialog::ZEtenControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZEtenControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZEtenControlDialog::~ZEtenControlDialog()
{
    delete ui;
}

void ZEtenControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZEtenControlDialog::updateList()
{
...
void ZEtenControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZEtenControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZEten_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZEtenControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZEtenControlDialog::GetSelectedMints()
{
...
void ZEtenControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
