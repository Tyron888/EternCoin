...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "etern.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "eternd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the ETEN and zETEN money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in ETEN/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in ETEN/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, etern, (obfuscation, swifttx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in ETEN/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Etern specific functionality (Masternodes, Zerocoin, SwiftTX, Budgeting) (0-1, default: %u)"), 0));
    ...
    // strUsage += HelpMessageOpt("-backupzeten=<n>", strprintf(_("Enable automatic wallet backups triggered after each zEten minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizeeternamount=<n>     " + strprintf(_("Keep N ETEN anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The Etern Core Developers"), "2021")) + "\n" +
           "\n" + 
...
/** Sanity checks
 *  Ensure that Etern is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Etern Core is shutting down."));
...
#endif
    // Make sure only a single Etern process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Etern Core is probably already running."), strDataDir));
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //Etern specific: zerocoin and spork DB's
                ...
                // Etern: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZETENMinted();
                        RecalculateZETENSpent();
                    }
                    RecalculateETENSupply(1);
                }
                ...
                // Etern: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Etern Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Etern Core to complete") << "\n";
        ...
        bool fEnableZEtenBackups = GetBoolArg("-backupzeten", true);
        pwalletMain->setZEtenAutoBackups(fEnableZEtenBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeEternAmount = GetArg("-anonymizeeternamount", 0);
//    if (nAnonymizeEternAmount > 999999) nAnonymizeEternAmount = 999999;
//    if (nAnonymizeEternAmount < 2) nAnonymizeEternAmount = 2;
    ...
    LogPrintf("Anonymize Etern Amount %d\n", nAnonymizeEternAmount);
    ...
    /* Denominations
       ...
       For example:
       1ETEN+1000 == (.1ETEN+100)*10
       10ETEN+10000 == (1ETEN+1000)*10
    */
